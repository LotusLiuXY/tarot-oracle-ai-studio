...
export default function CardsPage() {
  ...
  const punctuation = resolveStudioLocale(i18n.resolvedLanguage || i18n.language) === "zh-CN" ? "：" : ": ";

  return (
    <AppChrome>
      <div className="grid gap-5" data-el="cards-page">
        <section className="rounded-[36px] border border-border/50 bg-card/54 p-5 shadow-[var(--shadow-sm)] backdrop-blur-xl md:p-7" data-el="cards-hero">
        ...
        <section className="grid gap-5 lg:grid-cols-2" data-el="card-draft-grid">
          {data.cardDrafts.map((card) => (
            <article key={card.number} className="grid gap-4 rounded-[34px] border border-border/55 bg-card/52 p-4 shadow-[var(--shadow-sm)] backdrop-blur-xl md:grid-cols-[240px_1fr]" data-el="card-draft-item">
              ...
              <div className="flex min-w-0 flex-col justify-between gap-4">
                <div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className="rounded-full border border-border/60 bg-card/70 px-3 py-1 text-xs text-muted-foreground">{t("cards.count", { count: 17 })}</span>
                  </div>
                  ...
                  <div className="mt-4 grid gap-2 text-sm">
                    <p className="rounded-3xl bg-primary/10 p-3"><b className="text-secondary">{t("fields.image")}{punctuation}</b>{card.imageDescription}</p>
                    <p className="rounded-3xl bg-primary/10 p-3"><b className="text-secondary">{t("fields.upright")}{punctuation}</b>{card.upright}</p>
                    <p className="rounded-3xl bg-primary/10 p-3"><b className="text-secondary">{t("fields.reversed")}{punctuation}</b>{card.reversed}</p>
                  </div>
                </div>
