...
const FIELD_GROUPS: Record<FieldGroupKey, readonly string[]> = {
...
function getFieldValue(card: ReturnType<typeof getStudioData>["cardDrafts"][number], key: FieldKey) {
...
export default function Home() {
  ...
  const groupSummary = (group: FieldGroupKey) => FIELD_GROUPS[group].slice(0, 3).map((key) => fieldValues[key as FieldKey]).join(" · ");
    ...
    <AppChrome>
      <div className="grid gap-5" data-el="workspace-page">
        ...
        <section className="grid gap-5 xl:grid-cols-[1.08fr_.92fr]" data-el="workspace-main-grid">
          ...
          <div className="grid gap-5 min-w-0 content-start">
            <section className="rounded-[36px] border border-border/50 bg-card/54 p-4 shadow-[var(--shadow-sm)] backdrop-blur-xl" data-el="field-editor-panel">
              ...
              <div className="grid grid-cols-2 gap-3" data-el="field-group-grid">
                {(Object.keys(FIELD_GROUPS) as FieldGroupKey[]).map((group) => (
                  <FieldCard key={group} label={t(`fieldGroups.${group}`)} summary={groupSummary(group)} active={activeGroup === group} onClick={() => { setActiveGroup(group); setActiveField(FIELD_GROUPS[group][0] as FieldKey); }} />
                ))}
              </div>
              <div className="mt-4 flex min-w-0 gap-2 overflow-x-auto pb-2" data-el="field-tabs">
                {FIELD_GROUPS[activeGroup].map((key) => (
                  <button key={key} type="button" onClick={() => setActiveField(key as FieldKey)} className={cn("shrink-0 rounded-full border px-3 py-1.5 text-xs font-semibold transition", activeField === key ? "border-secondary bg-secondary text-accent" : "border-border/60 bg-card/70 text-muted-foreground")}>
                    {t(`fields.${key}`)}
                  </button>
                ))}
              </div>
